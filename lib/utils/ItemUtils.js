...
function getItemSprite (texture, item) {
  ...
  else if ([2, 5].includes(item.spread_type)) return texture.slices[item.texture_x + 4][item.texture_y + 1];
  else if ([3, 7, 8, 9].includes(item.spread_type)) return texture.slices[item.texture_x + 3][item.texture_y];
